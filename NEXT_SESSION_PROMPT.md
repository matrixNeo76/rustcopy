...
1. **`GenerationManifest`/`ScanSummary` tengono l'intero inventario in RAM come `Vec` e lo (de)serializzano per intero ad ogni run** — D19 (23 Agosto) ha chiuso solo il **costo di scrittura** del caso comune (NDJSON append-only invece di riscrivere tutto), non la lettura: `load_or_default` continua a caricare l'intera cronologia in RAM ad ogni run, e il pruning (`--keep-generations`) continua a riscrivere tutto per intero. A 174-872 MB per generazione a scala reale, resta aperta la domanda se serva un formato di lettura incrementale/streaming, non solo di scrittura — deliberatamente fuori scope per D19 (vedi `AskUserQuestion` in quella sessione: scope limitato al costo di scrittura, non alla dimensione/lettura).
