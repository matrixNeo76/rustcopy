...
1. **`ScanSummary` tiene l'intero inventario della sorgente in RAM come `Vec`** — la metà `GenerationManifest` di questo spunto è stata chiusa da D19 (costo di scrittura) e D20 (costo di lettura); resta aperta quella sullo *scan*, che costruisce comunque un `Vec<ScannedFile>` completo prima di copiare. **Non ancora misurata**: prima di toccarla, misurarla come è stato fatto per D20 (`probe_manifest_ram_at_real_world_scale` è il modello da riusare). Nota che D20 ha ridotto la **memoria**, non il tempo di parsing del manifest — `serde_json` deve comunque tokenizzare gli array `files` per saltarli; nessuna evidenza attuale giustifica di intervenire anche lì.
