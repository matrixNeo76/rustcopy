...
pub struct CopyRequestBuilder {
    ...
    inter_packet_gap_ms: Option<u32>,
    prescan: bool,
    long_paths: bool,
...
impl CopyRequestBuilder {
    ...
    pub fn file_retries(mut self, retries: u32) -> Self {
        self.file_retries = retries;
        self
    }
    ...
    pub fn build(self) -> CopyRequest {
        CopyRequest {
            source: self.source,
            dest: self.dest,
            pattern: self.pattern,
            threads: self.threads,
            file_retries: self.file_retries,
...
impl CopyRequest {
    pub fn builder(source: impl Into<PathBuf>, dest: impl Into<PathBuf>) -> CopyRequestBuilder {
        CopyRequestBuilder::new(source, dest)
    }
}
