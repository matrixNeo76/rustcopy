...
| `--purge-preview-path <PATH>` | *nessuno* | — | (F63) Scrive su `PATH` l'elenco JSON **completo** dei file che `--mirror` cancellerebbe, poi esce senza copiare né eliminare nulla. Richiede `--mirror`. Gira anche senza `--force-purge`: un'anteprima è una lettura, non un'autorizzazione. |
...
| `--list-schedules` | `false` | — | (F62) Elenca ogni voce di Task Scheduler il cui comando invoca questo binario, poi esce — nome, prossima esecuzione, stato e comando completo. Non richiede `--source`/`--dest`, come `--advise`. Di sola lettura: non installa, aggiorna o rimuove nulla. |
...
| `--progress-file <PATH>` | *nessuno* | — | Pubblica il progresso della run in un file che un supervisore può leggere: **una riga JSON riscritta in posto** al massimo una volta al secondo, con `atomic_write` (temp + rename), così chi legge non vede mai una riga a metà. La barra a terminale è disegnata con sequenze ANSI per una persona; un programma che guarda da un altro processo ha bisogno dei numeri. **Non è NDJSON in append** come storico e manifest: quelli servono la *storia*, il progresso serve *adesso*, e appendere lascerebbe un file che cresce per tutta la run con valori che nessuno rilegge. Il campione porta la **fase** (`inventory`/`transfer`/`verification`) oltre ai conteggi: una finestra che mostrasse solo i byte copiati resterebbe al 100% per tutta la verifica e sembrerebbe piantata. I totali sono **opzionali**: durante l'inventario non esistono, e renderli come 0% sarebbe un'invenzione. **Il file viene rimosso a fine run** — anche quando la run viene **fermata** — così l'assenza significa "non in corso" e chi legge non deve ragionare su quanto è vecchio il dato. Copre tutte e tre le pipeline, `--backup-type` compresa, che copia dal motore naive e non da `transfer()`. **Non pubblica durante `--pre-command`/`--post-command`**: gli hook girano fuori dalle tre fasi, quindi in quel tratto non c'è nulla da riferire. Un fallimento di scrittura **non fa mai fallire un backup** (regola 11): viene registrato una volta a `warn` e il publisher si ferma. **Limite dichiarato**: durante l'inventario non c'è conteggio file — `scan::scan` non riporta incrementalmente, e attraversare i suoi quattro call site per un numero non cambierebbe ciò che serve, cioè che la finestra dica cosa sta facendo e da quanto. |
| `--cancel-file <PATH>` | *nessuno* | — | Ferma la run quando questo file compare, **esattamente come farebbe `Ctrl+C`**: termina il figlio robocopy, scrive il checkpoint in `<report-path>.checkpoint.json` e esce. Per un supervisore senza terminale (la console desktop, un wrapper di servizio, un job CI): su Windows `GenerateConsoleCtrlEvent` richiede che chi chiama sia agganciato a una console e che il figlio stia in un proprio gruppo di processi, e uccidere il processo salterebbe il checkpoint — cioè proprio ciò che rende un'interruzione riprendibile con `--resume-from`. Le due sorgenti di interruzione confluiscono in **un solo** ramo di codice, non in due. Il percorso **non deve esistere** all'avvio: un file rimasto da una run precedente fermerebbe questa all'istante (exit code 2 con un messaggio che lo spiega). Il controllo è **una volta sola all'avvio**, non per singolo job: dentro un batch `[[jobs]]` un file di stop creato mentre il batch gira è un segnale legittimo, e il job in corso lo tratta come interruzione (checkpoint scritto, job successivi annullati, exit code 1). Verificarlo per job lo trasformava in un errore di configurazione per ciascun job rimanente, con exit code 2 e nessun checkpoint. Il polling è ogni 500 ms. **Limite noto**: se la run è dentro un `--pre-command`/`--post-command`, il checkpoint viene scritto subito ma il processo esce solo quando quel comando finisce — `kill_active_child` traccia il PID di robocopy, non quello degli hook. |
