...
> per cicli), F39 (comandi pre/post job) e F36 (scheduler via Task Scheduler) chiusi; F32, F37,
> F38, F40 restano da affrontare.
...
| **F37** | **Servizio Windows reale** | 🟠 P1 | Parità Cobian | Sostituisce il mock `service.rs`. F36 (chiuso) non ne ha più bisogno come prerequisito — resta un task a sé, dedicato principalmente al notify-server persistente (F41). |
...
`cargo test`: 262 (era 243, F36). `cargo test --features notify-server`: 275 (era 256).
