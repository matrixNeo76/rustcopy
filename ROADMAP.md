...
> verificati (5 Agosto 2026), F34 ora completo con Full+Incrementale+Differenziale, F35 (ritenzione
> per cicli), F39 (comandi pre/post job), F36 (scheduler via Task Scheduler) e F37 (servizio
> Windows reale, infrastruttura minima) chiusi; F32, F38, F40 restano da affrontare.
...
`cargo test`: 265 (era 262, F37). `cargo test --features notify-server`: 278 (era 275).
