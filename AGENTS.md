...
- All **307 unit and integration tests** (default build) MUST pass before committing changes. With `--features notify-server`, **322** must pass.
...
- The canonical graph is `graphify-out/graph.json` at the **repo root** — it covers the whole repo (code + docs + skills, not just `src/`), rebuilt with `/graphify . --mode deep` (last done 21 Agosto 2026, see `ANALYSIS.md` D10). Use `/graphify --update` to refresh it incrementally after code changes, **not** `graphify extract src/ ...` — that writes a second, divergent graph under `src/graphify-out/`, which is not covered by `.gitignore` and will silently drift from the real one.
