...
15. **Every permanent root-level `.md` file needs OKF frontmatter, enforced in CI**: a new file gets a minimal [OKF](https://github.com/W4G1/okf) v0.2 block (`type`/`title`/`description`/`status`/`generated`, `verified` only where something actually checked the claim) and an entry in the `TRACKED_DOCS` array in both steps of `.github/workflows/ci.yml`'s `docs` job — the first step (`Check no untracked root-level .md file`) fails CI if a root `.md` isn't in that list, the second (`Validate OKF frontmatter`) runs `okf parse` on it. This exists because `AGENT_HARNESS_PLAN.md` sat in root for 10 days with no frontmatter and no CI coverage before a manual audit caught it (20 Agosto 2026) — nothing had been checking for that class of drift. A one-off scratch file doesn't need any of this; a permanent doc does. `generated.at`/`verified.at` are provenance stamps, not "last edited" — OKF's own staleness model doesn't treat an old `generated.at` as a problem on a `stable` doc with no `ttl`, so don't bump it on every routine edit. Do bump `verified.at` (and `generated.at` if the rewrite is substantial enough to also count as regenerating the concept) when a `verified` document is rewritten enough that the original attestation no longer covers the new content — an unbumped `verified` after a substantial rewrite overclaims trust for content nobody re-checked (caught on `PIANO_MIGLIORAMENTI.md` in the same audit: `verified: 2026-08-17` was still standing after the whole B5 section was rewritten on 20 Agosto).
...
- All **302 unit and integration tests** (default build) MUST pass before committing changes. With `--features notify-server`, **317** must pass.
