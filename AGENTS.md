...
- All **506 unit and integration tests** (default build) MUST pass before committing changes. With `--features rustcopy-cli/notify-server`, **521** must pass. Exclude the GUI crate from the workspace run (`--exclude rustcopy-gui`), exactly as CI does; it has its own job.
